...
import { fetchReviewComments, postReview } from '../../api/api-action.ts';
...
export default function ReviewForm({offerId}: ReviewFormProps): JSX.Element {
  ...
  const handleSubmit = (evt: FormEvent<HTMLFormElement>) => {
    ...
    setRating('');
    setReviewText('');
    dispatch(fetchReviewComments({id: offerId}));
  };
  ...
  return (
    <form className="reviews__form form" onSubmit={handleSubmit} >
      ...
      <div className="reviews__button-wrapper">
        ...
        <button className="reviews__submit form__submit button" type="submit" disabled={rating === '' || reviewText.length < 50 || reviewText.length > 300 || authorizationStatus === AuthorizationStatus.NotAuthorized} >Submit</button>
      </div>
    </form>
  );
}
